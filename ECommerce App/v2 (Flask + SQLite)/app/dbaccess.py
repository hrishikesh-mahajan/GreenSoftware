...
def gen_custID():
    conn = sqlite3.connect("app/onlineshop.db")
    cur = conn.cursor()
...
def add_user(data):
    ...
    if data["type"] == "Customer":
        cur.execute(
            "INSERT INTO customer VALUES (?,?,?,?,?,?,?,?,?,?,?)", (gen_custID(), *tup)
        )
    elif data["type"] == "Seller":
        cur.execute(
            "INSERT INTO seller VALUES (?,?,?,?,?,?,?,?,?,?,?)", (gen_sellID(), *tup)
        )
    conn.commit()
    conn.close()
    return True
...
def auth_user(data):
    ...
    if type == "Customer":
        a = cur.execute(
            "SELECT custID, name FROM customer WHERE email=? AND password=?",
            (email, password),
        )
    elif type == "Seller":
        a = cur.execute(
            "SELECT sellID, name FROM seller WHERE email=? AND password=?",
            (email, password),
        )
    a = list(a)
    conn.close()
...
def fetch_details(userid, type):
    ...
    if type == "Customer":
        ...
        a = list(a)
        b = cur.execute(
            "SELECT DISTINCT(category) from product WHERE sellID=?", (userid,)
        )
        b = [i[0] for i in b]
    conn.close()
    return a, b
...
def search_users(search, srch_type):
    ...
    if srch_type == "Customer":
        res = cur.execute(
            "SELECT custID, name, email, phone, area, locality, city, state, country, zipcode FROM customer WHERE LOWER(name) like ?",
            (search,),
        )
    elif srch_type == "Seller":
        res = cur.execute(
            "SELECT sellID, name, email, phone, area, locality, city, state, country, zipcode FROM seller WHERE LOWER(name) like ?",
            (search,),
        )
    res = [i for i in res]
    conn.close()
...
def update_details(data, userid, type):
    ...
    if type == "Customer":
        cur.execute(
            "UPDATE customer SET phone=?, area=?, locality=?, city=?, state=?, country=?, zipcode=? where custID=?",
            (
                data["phone"],
                data["area"],
                data["locality"],
...
def set_psswd(psswd, userid, type):
    ...
    if type == "Customer":
        a = cur.execute(
            "UPDATE customer SET password=? WHERE custID=?", (psswd, userid)
        )
    elif type == "Seller":
...
def add_prod(sellID, data):
    ...
    cur.execute(
        "INSERT INTO product VALUES (?,?,?,?,?,(SELECT profit_rate from metadata)*?,?,?)",
        tup,
    )
    conn.commit()
    conn.close()
...
def search_myproduct(sellID, srchBy, category, keyword):
    ...
    if srchBy == "by category":
        a = cur.execute(
            """SELECT prodID, name, quantity, category, cost_price
                        FROM product WHERE category=? AND sellID=? """,
            (category, sellID),
        )
        res = [i for i in a]
...
def get_product_info(id):
    ...
    cur = conn.cursor()
    a = cur.execute(
        """SELECT p.name, p.quantity, p.category, p.cost_price, p.sell_price,
                    p.sellID, p.description, s.name FROM product p JOIN seller s
                    WHERE p.sellID=s.sellID AND p.prodID=? """,
        (id,),
    )
    res = [i for i in a]
    conn.close()
...
def get_seller_products(sellID):
    ...
    cur = conn.cursor()
    a = cur.execute(
        "SELECT prodID, name, category, sell_price FROM product WHERE sellID=? AND quantity!=0",
        (sellID,),
    )
    res = [i for i in a]
    conn.close()
...
def place_order(prodID, custID, qty):
    ...
    cur.execute(
        """INSERT INTO orders
                    ...
                    FROM product WHERE prodID=? """,
        (orderID, custID, prodID, qty, qty, qty, prodID),
    )
    conn.commit()
    conn.close()
...
def cust_orders(custID):
    ...
    cur = conn.cursor()
    a = cur.execute(
        """SELECT o.orderID, o.prodID, p.name, o.quantity, o.sell_price, o.date, o.status
...
def sell_orders(sellID):
    ...
    cur = conn.cursor()
    a = cur.execute(
        """ SELECT o.orderID, o.prodID, p.name, o.quantity, p.quantity, o.cost_price, o.date, o.status
                        FROM orders o JOIN product p
...
def get_order_details(orderID):
    ...
    cur = conn.cursor()
    a = cur.execute(
        """ SELECT o.custID, p.sellID, o.status FROM orders o JOIN product p
                        WHERE o.orderID=? AND o.prodID=p.prodID """,
        (orderID,),
    )
    res = [i for i in a]
    conn.close()
    ...
    if new_status == "DISPACHED":
        cur.execute(
            """UPDATE product SET
                     ...
                     WHERE prodID=(SELECT prodID FROM orders WHERE orderID=? )""",
            (orderID, orderID),
        )
    conn.commit()
    conn.close()
...
def cust_purchases(custID):
    ...
    cur = conn.cursor()
    a = cur.execute(
        """SELECT o.prodID, p.name, o.quantity, o.sell_price, o.date
                       FROM orders o JOIN product p
                       WHERE o.prodID=p.prodID AND o.custID=? AND o.status='RECIEVED'
                       ORDER BY o.date DESC """,
        (custID,),
...
def sell_sales(sellID):
    ...
    cur = conn.cursor()
    a = cur.execute(
        """SELECT o.prodID, p.name, o.quantity, o.sell_price, o.date, o.custID, c.name
