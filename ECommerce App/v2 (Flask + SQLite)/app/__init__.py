...
def profile():
    ...
    return render_template(
        "profiles.html", id=session["userid"], type=type, after_srch=False
    )
...
def my_products():
    ...
    if request.method == "POST":
        ...
        return render_template(
            "my_products.html", categories=categories, after_srch=True, results=results
        )
...
def buy_product(id):
    ...
    if request.method == "POST":
        ...
        return redirect(
            url_for("buy_confirm", total=total, quantity=data["qty"], id=id)
        )
    return render_template(
        "buy_product.html",
        name=name,
...
def cancel_order(orderID):
    ...
    return (
        redirect(url_for("my_orders"))
        if session["type"] == "Customer"
        else redirect(url_for("new_orders"))
    )
